...
class AuthService {
  ...
  Future<Map<String, dynamic>?> signUp({
    ...
    try {
      // 1∩╕ÅΓâú Create Firebase Auth user
      ...
      final userData = {
        ...
        'status': (role == 'Contractor' || role == 'Student')
            ? 'pending'
            : 'approved',
        'createdAt': FieldValue.serverTimestamp(),
      };

      // 2∩╕ÅΓâú Save extra data in Firestore
      ...
      // 3∩╕ÅΓâú Create public lookup entry for Students (to allow login by ID without public user-profile reads)
  ...
  Future<Map<String, dynamic>?> loginWithEmail({
    ...
    try {
      // 1∩╕ÅΓâú Login using email + password (directly)
      UserCredential cred = await _auth.signInWithEmailAndPassword(
        email: email,
        password: password,
      );

      // 2∩╕ÅΓâú Fetch user data by UID after login is successful
  ...
  Future<Map<String, dynamic>?> loginWithStudentId({
    ...
    try {
      // 1∩╕ÅΓâú Find email from public_lookups using studentId
      ...
      // 2∩╕ÅΓâú Login using email + password (directly)
      await _auth.signInWithEmailAndPassword(email: email, password: password);

      // 3∩╕ÅΓâú Fetch full user data after successful auth
  ...
  Future<Map<String, dynamic>?> loginWithPhone({
    ...
    try {
      // 1∩╕ÅΓâú Find email from Firestore using phone
      QuerySnapshot snap = await _db
          .collection('users')
      ...
      // 2∩╕ÅΓâú Login using email + password
      await _auth.signInWithEmailAndPassword(email: email, password: password);
  ...
  Future<void> updateFCMToken(String uid, String token) async {
    try {
      ...
      // Ignore if user document doesn't exist yet or other silent errors
    }
  }
